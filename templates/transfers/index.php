...
<div class="kommentarcontainer">
	<?php foreach ($comments as $kommentar): ?>
		<?php if($kommentar->theme == 4): ?>
		<div class="kommentarbox">
			<table style="width:100%">
				<tr>
					<th><div><b><?= $username; ?></b></div></th>
					<th id="th2"><div><a class="nolink" href="/transfers/delete?id=<?= $kommentar->id; ?>">Löschen</a></div></th>
					<th id="th3"><div><a class="nolink" href="/transfers/?id=<?= $kommentar->id; ?>">Bearbeiten</a></div></th>
				</tr>
			</table>
			...
			<?php if($edit_id != $kommentar->id): ?>
				<?= $kommentar->text ?>
			<?php else: ?>
				<form action="/transfers/doEdit?id=<?= $kommentar->id; ?>" method="post">
...
<div id="neuerKommentar">
    <form action="/transfers/doCreate" method="post">
