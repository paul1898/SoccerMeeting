...
<div class="kommentarcontainer">
	<?php foreach ($comments as $kommentar): ?>
		<?php if($kommentar->theme == 6): ?>
		<div class="kommentarbox">
			<table style="width:100%">
				<tr>
					<th><div><b><?= $username; ?></b></div></th>
					<th id="th2"><div><a class="nolink" href="/laliga/delete?id=<?= $kommentar->id; ?>">Löschen</a></div></th>
					<th id="th3"><div><a class="nolink" href="/laliga/?id=<?= $kommentar->id; ?>">Bearbeiten</a></div></th>
				</tr>
			</table>
			...
			<?php if($edit_id != $kommentar->id): ?>
				<?= $kommentar->text ?>
			<?php else: ?>
				<form action="/laliga/doEdit?id=<?= $kommentar->id; ?>" method="post">
...
<div id="neuerKommentar">
    <form action="/laliga/doCreate" method="post">
